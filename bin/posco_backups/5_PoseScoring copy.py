...
if __name__ == '__main__':
    ...
    for ts in u.trajectory[-args.n_frames:]:
        ...
        ligand_pdb = os.path.join(args.dir, f'ligand_{frame_count}.pdb')
        ligand.write(ligand_pdb)

        receptor_pdb = os.path.join(args.dir, f'receptor_{frame_count}.pdb')
        receptor.write(receptor_pdb)

        # Perform interaction analyzer
        interaction_csv = os.path.join(args.dir, 'lig', f'{frame_count}.csv')

        # Analyze interactions of the ligand
        command = f'po-sco {receptor_pdb} {ligand_pdb} -b > {interaction_csv}'
        execute(command)
