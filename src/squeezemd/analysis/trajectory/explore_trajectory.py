...
def visualize_MDStats(stats_file, output_graph):
    ...
    # Total Energy
    plt.subplot(2, 2, 1)
    sns.lineplot(data=data, x="time (ns)", y="Total Energy (kJ/mole)")

    plt.title("Total Energy")

    # Potential Energy
    plt.subplot(2, 2, 2)
    sns.lineplot(data=data, x="time (ns)", y="Potential Energy (kJ/mole)")

    plt.title("Potential Energy (kJ/mole)")

    # Temperature
    plt.subplot(2, 2, 3)
    sns.lineplot(data=data, x="time (ns)", y="Temperature (K)")

    plt.title("Temperature (K) Mean: " + str(data["Temperature (K)"].mean().round(2)))

    # Volume
    plt.subplot(2, 2, 4)
    sns.lineplot(data=data, x="time (ns)", y="Box Volume (nm^3)")
...
def visualize_RMSF(rmsf_data, secondary_structure_data, output_file):
    ...
    for chain, (resids, rmsf_values) in sorted(rmsf_data.items()):
        ...
        # Plot RMSF trace
        fig.add_trace(go.Scatter(x=resids, y=rmsf_values, mode="lines", name=f"Chain {chain} RMSF"), row=row, col=1)
        ...
        for helix in helix_series:
            ...
            fig.add_trace(
                go.Scatter(
                    x=[helix[0], helix[0], helix[1], helix[1]],
                    y=[floor_rmsf, ceil_rmsf, ceil_rmsf, floor_rmsf],
                    fill="toself",
                    fillcolor=helix_color,
        ...
        fig.update_layout(
            title="Toy Example with Shaded Area", xaxis_title="Residue number", yaxis_title="RMSF", showlegend=True
        )
    ...
    fig.update_layout(
        ...
        width=800,
        title_text="RMSF and Secondary Structure per Chain",
