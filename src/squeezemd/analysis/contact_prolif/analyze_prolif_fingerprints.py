...
def create_fig(fp_df, fig_path):
    ...
    df_grouped = (
        fp_df.groupby(["interaction_type", "mutation", "resid"])
        .agg(mean_sum=("sum", "mean"), std_sum=("sum", "std"))
        .reset_index()
    )
...
def parse_arguments():
    ...
    parser.add_argument(
        "--fingerprints",
        nargs="+",
        help="List of fingerprint parquet files",
        required=True,
        default=["fp1.parquet", "fp2.parquet"],
    )
    parser.add_argument(
        "--n_frames",
        help="How many frames to be analysed. Only the last n frames will be analyzed. Default 100",
        type=int,
        required=False,
        default=100,
    )
    ...
    # Output
    parser.add_argument(
        "--interactions", help="Joined fingerprint csv file path", required=False, default="fingerprint.csv"
    )
