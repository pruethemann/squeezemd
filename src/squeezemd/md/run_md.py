...
from openmm import CustomExternalForce, LangevinMiddleIntegrator, MonteCarloBarostat, OpenMMException, Platform, app
from openmm.unit import atmospheres, femtoseconds, kelvin, kilojoule_per_mole, molar, nanometers, picoseconds
...
def add_positional_restraints(
    ...
    for atom in topology.atoms():
        ...
        if verbose:
            print(
                f"Residue {resname} {resid} (index {resindex}), "
                f"Atom {atom.name}, element {atom.element.symbol}, "
                f"atom index {atom.index}"
            )
...
def simulate(args, params):
    ...
    system, restraint_force = add_positional_restraints(
        system, modeller.topology, modeller.positions, k=k, verbose=args.verbose, flexible_ligand=False
    )
    ...
    simulation = app.Simulation(
        modeller.topology, system, integrator, platform=platform, platformProperties=platform_properties
    )
    ...
    if "flexible_binding_pocket" in params["simulation"]:
        ...
        system, restraint_force = add_positional_restraints(
            ...
            modeller.positions,
            k=k,
            flexible_resids=flexible_resids,
...
def parse_arguments():
    ...
    parser.add_argument(
        "--metadynamics_hills", "--metadynamics_hill", dest="metadynamics_hills", help="Metadynamics hill output file."
    )
    parser.add_argument(
        "--metadynamics_colvar", default="output/metadynamics_colvar.txt", help="Metadynamics output file."
    )
