...
def compute_metadynamics(metadynamics_params, system):
    ...
    plumed.setTemperature(310*kelvin)
...
def simulate(args, params, salt_concentration=0.15):
    ...
    # ---------------------
    print('NVT heating with heavy atom restraints...')
    ...
    # ---------------------
    print('Switching to NPT for density equilibration...')
    ...
    for k in [5.0, 1.0]:
        ...
        for i in range(restraint_force.getNumParticles()):
            x0, y0, z0 = restraint_force.getParticleParameters(i)[1:]
            restraint_force.setParticleParameters(i, [k, x0, y0, z0])
